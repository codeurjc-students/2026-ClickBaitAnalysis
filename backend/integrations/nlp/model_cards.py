...
MODEL_CARDS: list[FichaModelo] = [
    {
        ...
        "limitations": [
            ...
            "Y la vía local, que es la única que queda, DEPENDE DE UN PAQUETE QUE `requirements.txt` NO TRAE: `torch`. No es un descuido —es lo que mantiene ligero al CI, que mockea los backends—, pero significa que una instalación de producción de hoy no puede ejecutar esta señal por ninguna de las dos vías hasta que la imagen instale torch. Medido el 2026-09-08: con la rueda CPU-only (769 MB) la señal responde sin GPU, con 1.201 MB de RAM para los tres modelos y 0.11 s por análisis en caliente.",
    ...
    },
    {
        ...
        "limitations": [
            ...
            "DEPENDE DE UN PAQUETE QUE `requirements.txt` NO TRAE: `sentence-transformers`. Y esta señal no tiene vía remota, así que en una instalación de producción de hoy falla con CUALQUIER `nlp_backend` — a diferencia de la dedicada, que al menos tiene dos caminos rotos por motivos distintos. Lo instala la imagen de despliegue; comprobado el 2026-09-08 que ponerlo después de la rueda CPU de torch no la sustituye por la variante CUDA.",
